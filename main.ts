...
input.onGesture(Gesture.EightG, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::EightG\n")
    }
})

input.onGesture(Gesture.FreeFall, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::FreeFall\n")
    }
})

input.onGesture(Gesture.LogoUp, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::LogoUp\n")
    }
})

input.onGesture(Gesture.TiltLeft, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::TiltLeft\n")
    }
})

input.onGesture(Gesture.SixG, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::SixG\n")
    }
})

input.onGesture(Gesture.ScreenUp, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::ScreenUp\n")
    }
})

input.onGesture(Gesture.ScreenDown, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::ScreenDown\n")
    }
})

input.onGesture(Gesture.Shake, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::Shake\n")
    }
})

input.onGesture(Gesture.TiltRight, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::TiltRight\n")
    }
})

input.onGesture(Gesture.LogoDown, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::LogoDown\n")
    }
})

input.onGesture(Gesture.ThreeG, function () {
    if (enable.gesture) {
        bluetooth.uartWriteString("" + commands.gesture + "::ThreeG\n")
    }
})
/////////////////////////////////////////////
...
input.onButtonPressed(Button.A, function () {
    if (enable.button_ab) {
        bluetooth.uartWriteString("" + commands.button_ab + "::a1\n")
    }
})

input.onButtonPressed(Button.B, function () {
    if (enable.button_ab) {
        bluetooth.uartWriteString("" + commands.button_ab + "::b1\n")
    }
})
///////////////////////////////////////////
...
input.onLogoEvent(TouchButtonEvent.Touched, function () {
    if (enable.logo) {
        bluetooth.uartWriteString("" + commands.logo + "::1\n")
    }
})

input.onLogoEvent(TouchButtonEvent.Released, function () {
    if (enable.logo) {
        bluetooth.uartWriteString("" + commands.logo + "::0\n")
    }
})
/////////////////////////////////////////////
...
bluetooth.onUartDataReceived(serial.delimiters(Delimiters.NewLine), function () {
    ...
    try {
        ...
        bluetooth.uartWriteString(response);
    } catch (e) {
        let response_error = function_name + ":" + function_id + ":-1\n";
        bluetooth.uartWriteString(response_error);
    } finally {
        readyForNextCommand = true; // set to true once processing is done
        basic.pause(100); // 100ms delay to provide buffer for Bluetooth
    }
})